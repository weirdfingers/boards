...
class TestStorageManager:
    ...
    @pytest.fixture
    def manager(self, config: StorageConfig) -> StorageManager:
    ...
    def test_init(self, manager: StorageManager, config: StorageConfig) -> None:
        assert manager.default_provider == "local"
        assert manager.config == config
        assert len(manager.providers) == 0

    def test_register_provider(self, manager: StorageManager, mock_provider: AsyncMock):
        manager.register_provider("test", mock_provider)
        assert "test" in manager.providers
        assert manager.providers["test"] == mock_provider

    def test_validate_storage_key_valid(self, manager: StorageManager):
        # Valid keys should pass
        assert manager._validate_storage_key("tenant/image/file.jpg") == "tenant/image/file.jpg"  # type: ignore
        assert manager._validate_storage_key("simple-file_name.ext") == "simple-file_name.ext"  # type: ignore

    def test_validate_storage_key_path_traversal(self, manager: StorageManager):
        # Path traversal attempts should fail
        with pytest.raises(SecurityException):
            manager._validate_storage_key("../etc/passwd")  # type: ignore

        with pytest.raises(SecurityException):
            manager._validate_storage_key("tenant/../other")  # type: ignore

        with pytest.raises(SecurityException):
            manager._validate_storage_key("/absolute/path")  # type: ignore

        with pytest.raises(SecurityException):
            manager._validate_storage_key("windows\\path")  # type: ignore

    def test_validate_content_type_allowed(self, manager: StorageManager):
        # Should not raise for allowed types
        manager._validate_content_type("image/jpeg")  # type: ignore
        manager._validate_content_type("text/plain")  # type: ignore

    def test_validate_content_type_disallowed(self, manager: StorageManager):
        with pytest.raises(ValidationException):
            manager._validate_content_type("application/executable")  # type: ignore

    def test_validate_file_size_ok(self, manager: StorageManager):
        # Should not raise for allowed sizes
        manager._validate_file_size(1024)  # 1KB  # type: ignore
        manager._validate_file_size(1024 * 1024)  # 1MB (limit)  # type: ignore
    ...
    def test_generate_storage_key_with_board(self, manager: StorageManager):
        key = manager._generate_storage_key(  # type: ignore
            artifact_id="test123",
            artifact_type="image",
            tenant_id="tenant1",
            board_id="board456",
    ...
    def test_generate_storage_key_without_board(self, manager: StorageManager):
        key = manager._generate_storage_key(  # type: ignore
            artifact_id="model789", artifact_type="model", tenant_id="tenant1"
        )
    ...
    def test_generate_storage_key_default_tenant(self, manager: StorageManager):
        key = manager._generate_storage_key(  # type: ignore
            artifact_id="test", artifact_type="image"
        )

        assert key.startswith("default/image/")

    def test_select_provider_default(self, manager: StorageManager):
        provider = manager._select_provider("image", b"small content")  # type: ignore
        assert provider == "local"

    def test_select_provider_with_rules(self, manager: StorageManager):
        config = StorageConfig(
            default_provider="local",
            providers={"local": {}, "s3": {}},
            routing_rules=[
                {"condition": {"artifact_type": "video"}, "provider": "s3"},
                {"provider": "local"},
            ],
        )
        ...
        assert manager._select_provider("video", b"content") == "s3"  # type: ignore

        # Other types should use default
        assert manager._select_provider("image", b"content") == "local"  # type: ignore

    def test_parse_size(self, manager: StorageManager):
        assert manager._parse_size("1024") == 1024  # type: ignore
        assert manager._parse_size("1KB") == 1024  # type: ignore
        assert manager._parse_size("1MB") == 1024 * 1024  # type: ignore
    ...
    @pytest.mark.asyncio
    async def test_store_artifact_validation_failure(
        self, manager: StorageManager, mock_provider: AsyncMock
    ):
        manager.register_provider("local", mock_provider)
    ...
    @pytest.mark.asyncio
    async def test_store_artifact_provider_not_found(self, manager: StorageManager):
