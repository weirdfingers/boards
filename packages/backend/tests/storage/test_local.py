...
class TestLocalStorageProvider:
    ...
    def test_init_with_public_url(self, temp_dir):
        provider = LocalStorageProvider(
            temp_dir, 
            public_url_base="http://example.com/files"
        )
        
        assert provider.public_url_base == "http://example.com/files"
    
    def test_get_safe_file_path_valid(self, provider, temp_dir):
    ...
    def test_get_safe_file_path_traversal_attack(self, provider):
    ...
    def test_get_public_url_with_base(self, provider):
        url = provider._get_public_url("folder/file.txt")
        assert url == "http://localhost:8000/storage/folder/file.txt"
    
    def test_get_public_url_with_special_chars(self, provider):
    ...
    def test_get_public_url_without_base(self, temp_dir):
        provider = LocalStorageProvider(temp_dir)
    ...
    @pytest.mark.asyncio
    async def test_upload_bytes(self, provider, temp_dir):
    ...
    @pytest.mark.asyncio
    async def test_upload_with_metadata(self, provider, temp_dir):
    ...
    @pytest.mark.asyncio
    async def test_upload_async_iterator(self, provider, temp_dir):
    ...
    @pytest.mark.asyncio
    async def test_upload_creates_directories(self, provider, temp_dir):
    ...
    @pytest.mark.asyncio
    async def test_download_success(self, provider, temp_dir):
        # Create test file
        key = "test/file.txt"
        content = b"test file content"
        file_path = temp_dir / key
    ...
    @pytest.mark.asyncio
    async def test_get_presigned_upload_url(self, provider):
    ...
    @pytest.mark.asyncio
    async def test_get_presigned_download_url(self, provider):
        url = await provider.get_presigned_download_url("test/file.txt")
        assert url == "http://localhost:8000/storage/test/file.txt"
    
    @pytest.mark.asyncio
    async def test_delete_success(self, provider, temp_dir):
        # Create test file
        key = "test/file.txt"
        file_path = temp_dir / key
        file_path.parent.mkdir(parents=True)
    ...
    @pytest.mark.asyncio
    async def test_delete_with_metadata(self, provider, temp_dir):
    ...
    @pytest.mark.asyncio
    async def test_exists_true(self, provider, temp_dir):
        # Create test file
        key = "test/file.txt"
        file_path = temp_dir / key
        file_path.parent.mkdir(parents=True)
    ...
    @pytest.mark.asyncio
    async def test_exists_path_traversal(self, provider):
        # Security violation should return False, not raise
        assert await provider.exists("../../../etc/passwd") is False
    
    @pytest.mark.asyncio
    async def test_get_metadata_success(self, provider, temp_dir):
    ...
    @pytest.mark.asyncio
    async def test_get_metadata_no_stored_metadata(self, provider, temp_dir):
