"""
Tests for FalMinimaxSpeech26TurboGenerator.
"""
...
class TestMinimaxSpeech26TurboInput:
    """Tests for MinimaxSpeech26TurboInput schema."""

    def test_valid_input(self):
        """Test valid input creation."""
        input_data = MinimaxSpeech26TurboInput(
        ...
        assert input_data.voice_setting.pitch == 0.5
        assert input_data.voice_setting.vol == 0.9
        assert input_data.language_boost == "en-US"
    ...
    def test_input_defaults(self):
        ...
        assert input_data.voice_setting.pitch == 0.0
        ...
        assert input_data.language_boost == "auto"
        assert input_data.output_format == "url"
    ...
    def test_prompt_length_validation(self):
        ...
        with pytest.raises(ValidationError):
            MinimaxSpeech26TurboInput(prompt="")

        # Test prompt exceeding maximum (10,000 characters)
        with pytest.raises(ValidationError):
            MinimaxSpeech26TurboInput(prompt="a" * 10001)

        # Test valid maximum length
        input_data = MinimaxSpeech26TurboInput(prompt="a" * 10000)
    ...
    def test_normalization_setting_validation(self):
        ...
        with pytest.raises(ValidationError):
            MinimaxSpeech26TurboInput(
                prompt="Test",
                normalization_setting=LoudnessNormalizationSetting(
                    target_range=-1.0  # Below minimum of 0
                ),
            )
        ...
        with pytest.raises(ValidationError):
            MinimaxSpeech26TurboInput(
                prompt="Test",
                normalization_setting=LoudnessNormalizationSetting(
                    target_range=25.0  # Above maximum of 20
                ),
            )
        ...
        with pytest.raises(ValidationError):
            MinimaxSpeech26TurboInput(
                prompt="Test",
                normalization_setting=LoudnessNormalizationSetting(
                    target_peak=-5.0  # Below minimum of -3
                ),
            )
        ...
        with pytest.raises(ValidationError):
            MinimaxSpeech26TurboInput(
                prompt="Test",
                normalization_setting=LoudnessNormalizationSetting(
                    target_peak=1.0  # Above maximum of 0
                ),
            )

    def test_voice_setting_options(self):
        """Test voice setting configuration."""
        input_data = MinimaxSpeech26TurboInput(
            prompt="Test",
            voice_setting=VoiceSetting(
        ...
        assert input_data.voice_setting.voice_id == "Custom_Voice"
        assert input_data.voice_setting.speed == 1.5
        assert input_data.voice_setting.pitch == 2.0
        assert input_data.voice_setting.vol == 0.8
        assert input_data.voice_setting.english_normalization is True
...
class TestFalMinimaxSpeech26TurboGenerator:
    """Tests for FalMinimaxSpeech26TurboGenerator."""
    ...
    def test_generator_metadata(self):
        ...
        assert self.generator.name == "fal-minimax-speech-2-6-turbo"
    ...
    def test_input_schema(self):
        ...
        assert schema_class == MinimaxSpeech26TurboInput
    ...
    async def test_estimate_cost_short_text(self):
        ...
        # Test with 71 characters
        prompt = "Hello world! This is a short test message for speech synthesis testing."
        input_data = MinimaxSpeech26TurboInput(
            prompt=prompt,
        )
        ...
        # Should be (71 / 1000) * 0.06 = 0.00426
        expected_cost = (len(prompt) / 1000.0) * 0.06
    ...
    async def test_estimate_cost_long_text(self):
        ...
        # Should be (5000 / 1000) * 0.06 = 0.30
        assert cost == 0.30
        assert isinstance(cost, float)

    def test_json_schema_generation(self):
        """Test that input schema can generate JSON schema for frontend."""
        schema = MinimaxSpeech26TurboInput.model_json_schema()
