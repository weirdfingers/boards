"""
Tests for FalFlux2FlexGenerator.
"""
...
class TestFlux2FlexInput:
    """Tests for Flux2FlexInput schema."""

    def test_valid_input(self):
        """Test valid input creation."""
        input_data = Flux2FlexInput(
    ...
    def test_input_defaults(self):
        """Test default values."""
        input_data = Flux2FlexInput(prompt="Test prompt")
    ...
    def test_num_images_validation(self):
        ...
        # Test below minimum
        with pytest.raises(ValidationError):
            Flux2FlexInput(prompt="Test", num_images=0)

        # Test above maximum
        with pytest.raises(ValidationError):
            Flux2FlexInput(prompt="Test", num_images=5)

        # Test valid range (1-4)
        for num in [1, 2, 3, 4]:
            input_data = Flux2FlexInput(prompt="Test", num_images=num)
    ...
    def test_guidance_scale_validation(self):
        ...
        # Test below minimum
        with pytest.raises(ValidationError):
            Flux2FlexInput(prompt="Test", guidance_scale=-1.0)

        # Test above maximum
        with pytest.raises(ValidationError):
            Flux2FlexInput(prompt="Test", guidance_scale=21.0)

        # Test valid range
        for scale in [0.0, 3.5, 10.0, 20.0]:
            input_data = Flux2FlexInput(prompt="Test", guidance_scale=scale)
    ...
    def test_num_inference_steps_validation(self):
        ...
        # Test below minimum
        with pytest.raises(ValidationError):
            Flux2FlexInput(prompt="Test", num_inference_steps=0)

        # Test above maximum
        with pytest.raises(ValidationError):
            Flux2FlexInput(prompt="Test", num_inference_steps=51)

        # Test valid range
        for steps in [1, 28, 50]:
            input_data = Flux2FlexInput(prompt="Test", num_inference_steps=steps)
    ...
    def test_image_size_options(self):
        ...
        for size in valid_sizes:
            input_data = Flux2FlexInput(prompt="Test", image_size=size)  # type: ignore[arg-type]
            assert input_data.image_size == size


class TestFlux2FlexGenerator:
    """Tests for FalFlux2FlexGenerator."""
    ...
    def test_generator_metadata(self):
        ...
        assert self.generator.name == "fal-flux-2-flex"
        assert self.generator.artifact_type == "image"
        assert "FLUX.2" in self.generator.description
        assert "flex" in self.generator.description.lower()

    def test_input_schema(self):
        """Test input schema."""
        schema_class = self.generator.get_input_schema()
        assert schema_class == Flux2FlexInput
    ...
    async def test_generate_with_custom_parameters(self):
        """Test generation with custom guidance and inference parameters."""
        input_data = Flux2FlexInput(
            prompt="Detailed artwork with text",
            guidance_scale=15.0,
    ...
    async def test_estimate_cost_single_image(self):
        ...
        input_data = Flux2FlexInput(prompt="Test prompt", num_images=1)
    ...
    async def test_estimate_cost_batch(self):
        ...
        input_data = Flux2FlexInput(prompt="Test prompt", num_images=4)

        cost = await self.generator.estimate_cost(input_data)

        assert cost == 0.20  # 4 images * $0.05
        assert isinstance(cost, float)

    def test_json_schema_generation(self):
        """Test that input schema can generate JSON schema for frontend."""
        schema = Flux2FlexInput.model_json_schema()
