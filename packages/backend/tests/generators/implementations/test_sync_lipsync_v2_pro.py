"""
Tests for FalSyncLipsyncV2ProGenerator.
"""
...
class TestSyncLipsyncV2ProInput:
    """Tests for SyncLipsyncV2ProInput schema."""
    ...
    def test_valid_input(self):
        ...
        input_data = SyncLipsyncV2ProInput(
...
class TestFalSyncLipsyncV2ProGenerator:
    """Tests for FalSyncLipsyncV2ProGenerator."""
    ...
    def test_generator_metadata(self):
        ...
        assert self.generator.name == "fal-sync-lipsync-v2-pro"
        assert self.generator.artifact_type == "video"
        assert "lipsync" in self.generator.description.lower()
        assert "pro" in self.generator.description.lower()

    def test_input_schema(self):
        """Test input schema."""
        schema_class = self.generator.get_input_schema()
        assert schema_class == SyncLipsyncV2ProInput
    ...
    @pytest.mark.asyncio
    async def test_generate_successful_with_loop_mode(self):
        """Test successful generation with loop sync mode."""
        video_artifact = VideoArtifact(
            generation_id="gen_input_video",
            storage_url="https://example.com/input-video.mp4",
            format="mp4",
        ...
        fake_output_url = "https://storage.googleapis.com/falserverless/output-loop.mp4"
        fake_uploaded_video_url = "https://fal.media/files/uploaded-video-loop.mp4"
        fake_uploaded_audio_url = "https://fal.media/files/uploaded-audio-loop.mp3"
        ...
        with patch.dict(os.environ, {"FAL_KEY": "fake-key"}):
            ...
            class DummyCtx(GeneratorExecutionContext):
                ...
                async def resolve_artifact(self, artifact):
                    if isinstance(artifact, VideoArtifact):
                        return "/tmp/fake_video_loop.mp4"
                    elif isinstance(artifact, AudioArtifact):
                        return "/tmp/fake_audio_loop.mp3"
                    return "/tmp/fake_file"
            ...
            # Verify API call used loop sync mode
    ...
    async def test_estimate_cost(self):
        ...
        # Pro model fixed cost
        assert cost == 0.10
        assert isinstance(cost, float)

    def test_json_schema_generation(self):
        """Test that input schema can generate JSON schema for frontend."""
        schema = SyncLipsyncV2ProInput.model_json_schema()
        ...
        assert "bounce" in sync_mode_prop["enum"]
        assert "silence" in sync_mode_prop["enum"]
        assert "remap" in sync_mode_prop["enum"]
