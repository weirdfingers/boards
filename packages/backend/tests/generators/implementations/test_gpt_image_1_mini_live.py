"""
Live API tests for FalGptImage1MiniGenerator.
...
To run these tests:
    ...
    pytest tests/generators/implementations/test_gpt_image_1_mini_live.py -v -m live_api

Or using direct environment variable:
    export FAL_KEY="..."
    pytest tests/generators/implementations/test_gpt_image_1_mini_live.py -v -m live_fal
...
class TestGptImage1MiniGeneratorLive:
    """Live API tests for FalGptImage1MiniGenerator using real Fal.ai API."""
    ...
    async def test_generate_basic(self, skip_if_no_fal_key, dummy_context, cost_logger):
        ...
        """
        # Log estimated cost
        estimated_cost = await self.generator.estimate_cost(GptImage1MiniInput(prompt="test"))
        cost_logger(self.generator.name, estimated_cost)

        # Create minimal input to reduce cost
        inputs = GptImage1MiniInput(
    ...
    async def test_generate_with_different_formats(
        ...
        estimated_cost = await self.generator.estimate_cost(
            GptImage1MiniInput(prompt="test", output_format="png")
        )
        cost_logger(self.generator.name, estimated_cost)
    ...
    async def test_estimate_cost_matches_batch_size(self, skip_if_no_fal_key):
        ...
        """
        # Single image
        inputs_1 = GptImage1MiniInput(prompt="test", num_images=1)
        cost_1 = await self.generator.estimate_cost(inputs_1)

        # Four images (maximum)
        inputs_4 = GptImage1MiniInput(prompt="test", num_images=4)
